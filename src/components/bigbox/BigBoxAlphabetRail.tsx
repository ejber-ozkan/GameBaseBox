...
export function BigBoxAlphabetRail({
  ...
  const isSearchRail = rail.id === 'search-results';
  const isRailActive = isActive || isSearchRail;

  const mediaWindow = useMemo(() => {
    const mediaEnabled = isRailActive && rail.games.length > 0 && (settledRailId === rail.id || isSearchRail);
    if (!isRailActive || !mediaEnabled) {
  ...
  }, [focusedIdx, isRailActive, isSearchRail, layout.gridColumns, rail.games.length, rail.id, settledRailId]);

  useEffect(() => {
    if (!isRailActive || rail.games.length === 0) {
      return;
    }
  ...
  }, [isRailActive, isSearchRail, rail.games.length, rail.id]);

  return (
    <div
      data-rail-id={rail.id}
      className={`flex flex-col transition-all duration-700 scroll-mt-[340px] ${isRailActive ? 'opacity-100' : 'opacity-25 translate-y-4'}`}
    ...
    >
      <div data-rail-anchor className="flex items-center gap-4" style={{ paddingLeft: `${layout.railPaddingX}px`, paddingRight: `${layout.railPaddingX}px` }}>
        <h2
          className={`font-black uppercase tracking-tighter ${isRailActive ? 'text-[var(--theme-primary)]' : 'text-[var(--theme-text-muted)]'}`}
