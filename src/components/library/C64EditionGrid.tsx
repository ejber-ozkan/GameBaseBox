...
export function C64EditionGrid({
  ...
  return (
    <div
      className="c64-edition-grid grid gap-8 px-4 pb-24 pt-6 sm:px-6 lg:grid-cols-[minmax(0,1fr)] lg:px-8"
      data-c64-presentation="monitor"
      data-testid="c64-edition-grid"
    >
      <section className="c64-recent-chips">
        <div className="mb-4 flex items-center gap-3">
          <div className="h-5 w-5 bg-[var(--theme-primary)]" />
          <h2 className="font-mono text-xl font-black tracking-tight text-[var(--theme-primary)] sm:text-2xl">RECENT_CHIPS</h2>
        ...
        {recentGames.length > 0 ? (
          ...
          <div className="border-4 border-dashed border-[var(--theme-outline-variant)] p-5 font-mono text-sm text-[var(--theme-text-muted)]">NO RECENT_CHIPS LOADED.</div>
        )}
      </section>
      ...
      <section className="c64-library-romset">
        <div className="mb-4 flex items-center gap-3">
          ...
          <h2 className="font-mono text-xl font-black tracking-tight text-[var(--theme-secondary)] sm:text-2xl">LIBRARY_ROMSET</h2>
          <div className="h-px flex-1 bg-[var(--theme-outline-variant)]" />
        </div>
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8">
          {games.map((game, index) => {
            ...
            return (
              <article
                key={`${game.id}-${index}`}
                className={`group relative cursor-pointer border-4 bg-black p-1 transition-colors ${
                  focused
                    ? 'border-[#ffff66] bg-[var(--theme-primary-container)] shadow-[0_0_0_2px_#ffff66]'
                    : 'border-[var(--theme-outline-variant)] hover:border-[var(--theme-primary)]'
                }`}
              ...
              >
                ...
                <div className="mt-1 flex items-start justify-between gap-1 px-1">
                  ...
                  <button
                    ...
                    className="shrink-0 font-mono text-sm text-[var(--theme-tertiary)]"
                    onClick={(event) => {
