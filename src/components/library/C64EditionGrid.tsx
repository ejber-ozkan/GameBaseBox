  ...
  return (
    <div className="c64-edition-grid grid gap-8 px-4 pb-24 pt-6 sm:px-6 lg:grid-cols-[minmax(0,1fr)] lg:px-8" data-c64-presentation="monitor" data-testid="c64-edition-grid">
      ...
      {librarySections.map((section, sectionIndex) => {
        const targetRailId = activeAlphabetRailId ?? alphabetSections?.[0]?.id;
        const shouldRender = isSearching || targetRailId === undefined || targetRailId === section.id || section.id === 'search-results';
        const displayGames = shouldRender ? section.games : [];
        ...
        return (
          <section key={section.id} className="c64-library-romset" data-rail-id={section.id}>
            ...
            <div
              ...
              style={gridColumns && gridColumns > 2 ? { gridTemplateColumns: `repeat(${gridColumns}, minmax(0, 1fr))` } : undefined}
            >
