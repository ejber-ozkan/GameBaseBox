...
describe('CyberpunkCrtGrid', () => {
  ...
  it('applies gridTemplateColumns when gridColumns > 2', () => {
    render(
      <CyberpunkCrtGrid
        games={[mockGames[0], mockGames[1]]}
        recentGames={[]}
    ...
    const libraryGrid = screen.getAllByTestId('cyberpunk-library-card')[0]?.parentElement;
    expect(libraryGrid?.getAttribute('style') ?? '').toContain('repeat(5, minmax(0, 1fr))');
  });
  ...
  it('omits RECENT, FAVOURITE, and CLASSICS rails when a search query is active so search results display at the top', () => {
    render(
      <CyberpunkCrtGrid
        activeAlphabetRailId="search-results"
        alphabetSections={[{ id: 'search-results', label: 'Results for "pole"', games: [mockGames[0]] }]}
        games={[mockGames[0]]}
        recentGames={[]}
        favoriteGames={[]}
        classicGames={[]}
    ...
    expect(screen.queryByRole('heading', { name: 'FAVOURITE' })).toBeNull();
    expect(screen.queryByRole('heading', { name: 'CLASSICS' })).toBeNull();
    expect(screen.getByRole('heading', { name: 'Results for "pole"' })).toBeTruthy();
    expect(screen.getByTestId('cyberpunk-library-card')).toBeTruthy();
  });

  it('renders search result cards even when activeAlphabetRailId is null (header is focused)', () => {
    render(
      <CyberpunkCrtGrid
        activeAlphabetRailId={null}
        alphabetSections={[{ id: 'search-results', label: 'Results for "pole"', games: [mockGames[0]] }]}
        games={[mockGames[0]]}
        recentGames={[]}
        favoriteGames={[]}
  ...
  it('renders only the first alphabet section cards when activeAlphabetRailId is null without search (preventing full library DOM explosion)', () => {
    render(
      <CyberpunkCrtGrid
        activeAlphabetRailId={null}
        alphabetSections={[
          ...
          { id: 'alpha-B', label: 'B', games: [{ ...mockGames[2], name: 'Boulder Dash' }] },
        ]}
        games={[]}
        recentGames={[]}
        favoriteGames={[]}
    ...
    // Only alpha-# cards should render (1 card), not alpha-A or alpha-B cards
    expect(screen.getAllByTestId('cyberpunk-library-card')).toHaveLength(1);
    expect(screen.getByText('1942')).toBeTruthy();
    expect(screen.queryByText('Archon')).toBeNull();
    expect(screen.queryByText('Boulder Dash')).toBeNull();
  });
});
