...
describe('getC64ViewingPath', () => {
  ...
  it('formats search input correctly when at least two characters are entered', () => {
    expect(getC64ViewingPath(undefined, 'm')).toBe('LIST INDEX_ALL');
    expect(getC64ViewingPath(undefined, 'mario')).toBe('LIST SEARCH: MARIO');
    expect(getC64ViewingPath('', 'Zelda')).toBe('LIST SEARCH: ZELDA');
  });

  it('combines letter and search input correctly when at least two characters are entered', () => {
    expect(getC64ViewingPath('M', 'm')).toBe('LIST INDEX_M');
